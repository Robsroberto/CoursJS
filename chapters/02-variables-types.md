...
```code Variables : En JavaScript, les variables sont déclarées à l'aide des mots-clés var, let. Elles sont utilisées
```

pour stocker des valeurs qui peuvent être modifiées au cours de l'exécution du programme. Exemple :

```javascript var age = 25; let nom = "Jean"; Constantes : Les constantes sont déclarées avec le mot-clé const et ne peuvent pas être réaffectées
```

après leur initialisation. Exemple :
...
```javascript var : La portée des variables déclarées avec var est fonctionnelle, ce qui signifie qu'elles peuvent être
```

accessibles même en dehors de leur bloc de code parent.

```javascript let : La portée des variables déclarées avec let est limitée au bloc de code dans lequel elles sont
```

déclarées.

```javascript const : Les constantes déclarées avec const ne peuvent pas être réaffectées après leur initialisation et
```
...
2. console.error() : Utilisée pour afficher des messages d'erreur dans la console. Exemple :

```code console.error("Une erreur s'est produite!");
```

3. console.warn() : Utilisée pour afficher des messages d'avertissement dans la console. Exemple :

```code console.warn("Attention : cette action est risquée!");
```

4. console.info() : Utilisée pour afficher des messages informatifs dans la console. Exemple :

```code console.info("Informations : ce site utilise des cookies.");
```
...
En JavaScript, les variables peuvent stocker différents types de données. Voici les principaux types de données en JavaScript : 1. Nombre (Number) : Représente les nombres entiers ou décimaux. Exemples :

```javascript let age = 30; // entier let prix = 19.99; // décimal
```

2. Chaîne de caractères (String) : Représente une séquence de caractères. Exemple :

```javascript let nom = "Jean";
```

3. Booléen (Boolean) : Représente une valeur vraie (true) ou fausse (false). Exemple :
...
5. Indéfini (Undefined) : Représente une variable qui n'a pas encore été affectée ou à laquelle aucune valeur n'a été attribuée. Exemple :

```javascript let prenom;
```

6. Objet (Object) : Représente une collection de données et/ou de fonctionnalités. Exemple :

```javascript let personne = { nom: "Alice", age: 25 };
```

7. Tableau (Array) : Représente une collection ordonnée d'éléments. Exemple :

```javascript let nombres = [1, 2, 3, 4, 5];
```
...
**Conversion de Types :** JavaScript est un langage de typage dynamique, ce qui signifie que les types de données peuvent être convertis implicitement ou explicitement. Voici quelques exemples de conversion de types : Conversion explicite :
...
```javascript let estMajeur = (age >= 18) ? "majeur" : "mineur"; console.log("Vous êtes " + estMajeur + ".");
```
...
```code 1. Déclarez une variable age et initialisez-la avec votre âge. 2. Déclarez une constante PI et initialisez-la avec la valeur de π (pi). 3. Affichez la valeur de age dans la console. 4. Essayez de réaffecter une nouvelle valeur à PI et observez ce qui se passe.
```

Exercice 2 : Utilisation des Opérateurs

```code 1. Déclarez deux variables x et y et initialisez-les avec des valeurs numériques.
```

2. Utilisez les opérateurs arithmétiques pour effectuer les opérations suivantes :

```code Additionnez x et y. Soustrayez y de x. Multipliez x par y. Divisez x par y. Calculez le reste de la division de x par y.
```

3. Affichez les résultats de chaque opération dans la console. Exercice 3 : Structures Conditionnelles

```code 1. Demandez à l'utilisateur son âge à l'aide de la fonction prompt() et stockez la valeur dans une variable. 2. Utilisez une structure conditionnelle if...else pour afficher un message différent en fonction de l'âge
```

de l'utilisateur : Si l'âge est supérieur ou égal à 18, affichez "Vous êtes majeur." Sinon, affichez "Vous êtes mineur." Exercice 4 : Structures Répétitives

```code 1. Utilisez une boucle for pour afficher les nombres de 1 à 10 dans la console. 2. Utilisez la boucle for pour afficher la table de multiplication d'un nombre recupéré avec promt() 3. Utilisez une boucle while pour afficher les nombres pairs de 2 à 10 dans la console. 4. Utilisez une boucle do...while pour demander à l'utilisateur de deviner un nombre entre 1 et 10.
```
